...
Force the UI text mirroring direction to be the direction (either `ltr` for left-to-right mirroring or `rtl` for right-to-left mirroring) specified. This does not affect the direction of static pages as that is specified by the [`--lang` argument](#--lang).
